...
#    links = links.reindex(shuffle(np.array(links.index)))
    ...
    with open(path + output_path + filename + 'Reslog.txt', 'w') as text_file:
...
#    print('AUC: %.3f' % auc)
    ...
    plt.savefig(path + output_path + filename + '_ROC.png')
    plt.close()
    ...
    plt.savefig(path + output_path + filename + '_PR.png')
    plt.close()
