...
if __name__ == '__main__':
    ...
    for file in file_list:
        ...
        Node2Vec_dict_randm = {
            'randm1': compare.repeat_node2vec(Sim, Sim_Ref, Sim_Ref_dropout, p, q, top_edges, param_grid,
                                              use_ref=False),
            'randm2': compare.repeat_node2vec(Sim, Sim_Ref, Sim_Ref_dropout, q, p, top_edges, param_grid,
                                              use_ref=False)}
        ...
        res.to_csv(output + file + '_performance.csv')

    performance_res.to_csv(output + 'HSC_MEP_2_Performance.csv')
...
performance_res = pd.read_csv(os.getenv(
    'HOME') + '/MING_V9T/PhD_Pro/Test/Simulation/Node2Vec_BoolODE_Res/Node2Vec_BoolODE_Res_Repeat_Jun2020/HSC_MEP_2_Performance/HSC_MEP_2_Performance.csv')
order = ['GENIE3', 'PIDC', 'GRNBOOST2', 'NODE2VEC', 'NODE2VEC_REF']
...
sns.boxplot(y='AUROC', x='AlgName', hue='Dropout',
            data=performance_res,
            ax=axs[0], order=order, palette='Set1', hue_order=hue_order).set(title='HSC_MEP')
...
sns.boxplot(y='AUPRC', x='AlgName', hue='Dropout',
            data=performance_res,
            ax=axs[1], order=order, palette='Set1', hue_order=hue_order).set(title='HSC_MEP')
# # get legend information from the plot object
l = ax2.legend(handles[0:4], labels[0:4])
ax2.set_xticklabels(ax2.get_xticklabels(), rotation=45)
fig.tight_layout()
fig.savefig(os.getenv('HOME') + '/Desktop/HSC_MEP_2_Performance_2.pdf')
