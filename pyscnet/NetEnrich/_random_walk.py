...
def greedy_walk(gnetdata, start, supervisedby, steps):
    """
    ToDo: Test!!!
    Given a network and node centralities, performs a greedy walk guided by node centrality.
    """

    if supervisedby not in ['betweenness', 'closeness', 'degree', 'pageRank']:
        raise Exception("Valid supervisedby options: betweenness, closeness, degree, pageRank")
    ...
    for _ in range(steps):
        ...
        remove_traverse = set(neighbors.keys()) - set(path)
        ...
        new_neighbors_score = {
            key: float(supervise_matrix.loc[supervise_matrix['node'] == key, supervisedby]) * float(list(score.values())[0])
            for key, score in new_neighbors.items()
        }

        go = max(new_neighbors_score, key=new_neighbors_score.get)
        path.append(go)
        start = go
